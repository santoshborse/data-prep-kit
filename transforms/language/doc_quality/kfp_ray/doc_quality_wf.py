...
TASK_NAME: str = "doc_quality"


@dsl.pipeline(
    name=TASK_NAME + "-ray-pipeline",
    description="Pipeline for multiple doc_quality",
)
def doc_quality(
    # Ray cluster
    ...
    with dsl.ExitHandler(clean_up_task):
        ...
        execute_job = execute_ray_jobs_op(
            ...
            run_id=run_id,
            additional_params=additional_params,
            # note that the parameters below are specific for Document Quality transform
...
if __name__ == "__main__":
    ...
    compiler.Compiler().compile(doc_quality, __file__.replace(".py", ".yaml"))
