...
TASK_NAME: str = "doc_chunk"


@dsl.pipeline(
    name=TASK_NAME + "-ray-pipeline",
    description="Pipeline for doc_chunk",
)
def doc_chunk(
    # Ray cluster
    ...
    with dsl.ExitHandler(clean_up_task):
        ...
        execute_job = execute_ray_jobs_op(
            ...
            run_id=run_id,
            additional_params=additional_params,
            # note that the parameters below are specific for chunk documents transform
...
if __name__ == "__main__":
    ...
    compiler.Compiler().compile(doc_chunk, __file__.replace(".py", ".yaml"))
